import { IUpdatePaymentModel, validateBodyFields } from '../add-payment/add-payment-protocols'
import {
  IController,
  IHttpRequest,
  IHttpResponse,
    ...
    try {
      ...
      const data = validateBodyFields<IUpdatePaymentModel>(
        [
          ...
          { key: 'hasDeleted', required: false, type: 'boolean' },
          { key: 'hasPaid', required: false, type: 'boolean' },
          { key: 'paidAt', required: false, type: 'string' }
        ],
        httpRequest.body
      )

      const response = await this.updatePaymentByIdUseCase.updateById(params.id, data)
